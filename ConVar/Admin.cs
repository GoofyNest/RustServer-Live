...
public class Admin : ConsoleSystem
{
	...
	public static void status(Arg arg)
	{
		...
		if (!flag && @string.Length == 0)
		{
			...
			text = text + "version : " + 2360 + " secure (secure mode enabled, connected to Steam3)\n";
	...
	{
		if (allowAdminUI && !(arg.Player() == null))
		{
			...
			if (array != null)
			{
				...
				arg.Player().ClientRPCEx(sendInfo2, null, "AdminReceivedUGC", uInt, (uint)array.Length, array, uInt3, (byte)@int);
			}
		}
	}
